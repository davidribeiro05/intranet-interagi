...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t davidribeiro05/intranet-interagi-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t davidribeiro05/intranet-interagi-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name intranet-interagi-backend-acceptance -d davidribeiro05/intranet-interagi-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name intranet-interagi-frontend-acceptance --link intranet-interagi-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d davidribeiro05/intranet-interagi-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop intranet-interagi-frontend-acceptance
	@docker stop intranet-interagi-backend-acceptance
